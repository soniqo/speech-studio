...
- **v0: Windows / Linux (x86_64)** — `VoxCPM2` and `Indic-Mio` via speech-core's LiteRT backend (`core-sidecar`). Same clone + emotion-marker story without MLX; synthesis is single-shot on every platform (see Notes).
...
- **speech-swift** — speech models runtime for Apple Silicon (MLX / CoreML). The macOS voice-cloning backend: `CosyVoiceTTS` (default), `VoxCPM2TTS`, `Qwen3TTS` (ICL clone API in `Qwen3TTS+ICL.swift`), `ChatterboxTTS` (multilingual, `Sources/ChatterboxTTS/`), `OmniVoiceTTS`, `IndicMioTTS`, and `FishAudioTTS`.
...
To rebuild a tag's release artifacts later (GitHub stops allowing re-runs after 30 days),
re-dispatch the workflow on the tag: `gh workflow run build.yml --ref vX.Y.Z`, then approve
the `release` environment gate when the macOS job pauses.
...
- **Windows/Linux sidecar (`core-sidecar`)**: `cfg`'d off macOS. On `init_model` it loads the selected bundle from `SONIQO_VOXCPM2_BUNDLE_DIR` / `SONIQO_INDIC_MIO_BUNDLE_DIR` if set, else calls `sc_voxcpm2_create_from_pretrained` or `sc_indic_mio_create_from_pretrained` to download+cache it (`SONIQO_VOXCPM2_MODEL_ID`, default `soniqo/VoxCPM2-LiteRT`; `SONIQO_INDIC_MIO_MODEL_ID`, default `soniqo/Indic-Mio-LiteRT`; cache via `SONIQO_MODEL_CACHE_DIR`/`SPEECH_CORE_CACHE_DIR`). The CMake colocates `libLiteRt` next to the binary; libcurl is linked statically (no extra DLL). The shell's fixed `cfgValue` has no LiteRT knob and is ignored; `seed` is honored (Regenerate draws a fresh one).
- **TTS model**: macOS defaults to the CosyVoice3 MLX engine (bf16); VoxCPM2 (`aufklarer/VoxCPM2-MLX-bf16`, int8 alternate) and Indic-Mio (`aufklarer/Indic-Mio-MLX-fp16`) are selectable. Engines with several published weights list them under `variants` in `model-registry.json` (VoxCPM2 bf16/int8, CosyVoice 3 bf16/8-bit, Qwen3-TTS bf16/8-bit); the entry's top-level `modelId` is the default and must match one variant. The user's pick persists in `<app-data>/model-settings.json`, is resolved in Rust, and rides the `modelId` field of `init_model` / every synth request — the sidecar holders reload when the id changes, so a platform override that swaps runtimes must also supply (or drop) `variants`. Windows/Linux use the `VoxCPM2-LiteRT` or `Indic-Mio-LiteRT` bundles — downloaded on first run (resumable and parallelized for large files; see speech-core's `SPEECH_CORE_WITH_HF_DOWNLOAD`) or supplied via the bundle-dir env vars.
...
- **Activity log**: everything the sidecar prints on stderr plus the shell's own `studio_log!` lines goes through one bounded ring (`ACTIVITY_LOG` in `lib.rs`): streamed to the WebView as `activity_log` events (the bottom **Activity** panel, toggled from the transport bar), appended to `<app-log-dir>/speech-studio.log` (rotated at 5 MB; `~/Library/Logs/audio.soniqo.studio/` on macOS), and still echoed to stderr. Use `studio_log!` instead of `eprintln!` in the shell so lines reach the panel. The Swift sidecar's `[sidecar] mem <label> active=..M cache=..M peak=..M rss=..M footprint=..M` snapshots (after every load and render) are parsed into `sidecar_memory` events for the panel's memory readout; the C++ sidecar doesn't emit them yet.
- **Follow-ups**: (1) Synthesis is single-shot on every platform — no seed/cfg retry ladder and no ASR grading gate (Parakeet cannot read non-Latin scripts and burned every seed on good Hindi/CJK takes); an unlucky take is re-rolled with Regenerate, which draws a fresh seed. (2) The Windows/Linux CI lanes (`build.yml` `build-windows` / `build-linux`) build the sidecar, run `ctest`, `pnpm tauri build`, and upload the installers as artifacts; on tag, the `publish-installers` job attaches them to the Release only after the signed macOS lane has succeeded, so a release can't go live without its `.dmg` + `latest.json` — but they check out speech-core at the moving `ref: main` (not a pinned SHA) and run no model inference, so an x86_64 LiteRT runtime regression can still slip past CI. First-run model download is handled by speech-core, so installers don't embed the bundle.
